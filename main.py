...
def create_dataset(path1,path2, num_examples):
    lines1 = open(path1, encoding='UTF-8').read().strip().split('\n')
    lines2 = open(path2, encoding='UTF-8').read().strip().split('\n')
    lines = []
    for i in range(len(lines1)):
        lines.append(lines1[i][:10]+"\t"+lines2[i][:10])
    word_pairs = [[preprocess_sentence(w) for w in l.split('\t')] for l in lines[:num_examples]]
...
def load_dataset(path, num_examples):
    ...
    pairs = create_dataset('./Data/train_input_text','./Data/train_output_text', num_examples)
...
#translate(u'Hey', encoder, decoder, inp_lang, targ_lang, max_length_inp, max_length_targ)
...
while user_input!='Q':
    user_input = input('> ')
    translate(user_input, encoder, decoder, inp_lang, targ_lang, max_length_inp, max_length_targ)